...
use tlms::grpc::receive_waypoint_server::{ReceiveWaypointServer, ReceiveWaypoint};
...
pub type R2D2Pool = ClusterClient;
...
pub struct Bureaucrat {
    redis_connection_pool: R2D2Pool
}

impl Bureaucrat {
    fn new() -> Option<Bureaucrat> {
        connect_to_redis().map(|pool| {
            Bureaucrat { redis_connection_pool: pool } 
        })
    }
}
...
impl ReceiveWaypoint for Bureaucrat {
    async fn receive_waypoint(
        ...
        let waypoints_strings: Vec<String> = match redis_connection.get(format!("r{}", extracted.region)) {
            Ok(value) => value,
            Err(_) => Vec::new()
        };
        ...
        let string_waypoints : Vec<String> = waypoints.iter().map(|x| serde_json::to_string(x).unwrap()).collect();
        
        let key: String = format!("r{}", region);
        match redis::cmd("SET").arg(key).arg(string_waypoints).query(&mut redis_connection) {
...
async fn main() -> std::io::Result<()> {
    ...
    info!("the bureaucrat receives its papaer work on {} ... ", &grpc_bureaucrat_host);
